---
title: "Preregistered pre-processing for E1"
...
files <- list.files(here("raw_data/combined pilot"), pattern = "*.json",
...
raw_data <- map_df(files, read_file) %>%
  mutate(order = if_else(str_detect(file, "combined pilot/1") | str_detect(file, "combined pilot/3"), 
                         "unknown first", "mixed first"),
         mixed_repeated = if_else(str_detect(file, "combined pilot/3") | str_detect(file, "combined pilot/4"), 
                         "known repeated", "unknown repeated")) %>%
  select(mixed_repeated, order, id, trial, target_type, distractor_type, response, time_elapsed, target, distractor, file, time_elapsed)
```


```{r times, eval = FALSE}
raw_data %>%
  group_by(mixed_repeated, order, id, file) %>%
  summarise(time_elapsed = sum(time_elapsed)/10000/60) %>%
  filter(time_elapsed >= 80)
...
write_csv(pre_processed_data, here("data/combined/pre_processed_data.csv"))

to_code_data <- pre_processed_data %>%
  select(mixed_repeated, order, id, trial, response, negation)

write_csv(to_code_data, here("data/combined/to_code_data.csv"))
```
