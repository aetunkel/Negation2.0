...
files <- list.files(here("raw_data/pilots/combined-pilot"), pattern = "*.json",
...
raw_data <- map_df(files, read_file) %>%
  mutate(order = if_else(str_detect(file, "combined-pilot/1") | 
                           str_detect(file, "combined-pilot/3"), 
                         "unknown first", "mixed first"),
         mixed_repeated = if_else(str_detect(file, "combined-pilot/3") | 
                                    str_detect(file, "combined-pilot/4"), 
...
write_csv(pre_processed_data, here("data/pilots/combined/pre_processed_data.csv"))

to_code_data <- pre_processed_data %>%
  select(mixed_repeated, order, id, trial, response, negation)

write_csv(to_code_data, here("data/pilots/combined/to_code_data.csv"))
```
