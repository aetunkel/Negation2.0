...
files <- list.files(here("raw_data/pre-reg1"), pattern = "*.json",
...
raw_data <- map_df(files, read_file) %>%
  mutate(order = if_else(str_detect(file, "unkFirst"), "unknown first", "known first"),
         experiment = if_else(str_detect(file, "Study1a"), "1a", "1b")) %>%
  select(experiment, order, id, trial, type, response, time_elapsed, target, distractor)
```
...
write_csv(pre_processed_data, here("data/pre-reg1/pre_processed_data.csv"))

to_code_data <- pre_processesd_data %>%
  select(experiment, id, trial, response, negation)

write_csv(to_code_data, here("data/pre-reg1/to_code_data.csv"))
```
