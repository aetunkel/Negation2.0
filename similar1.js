...
var test = {
	  ...
	  on_finish: function(){
	    trialnum = trialnum + 1;
        jsPsych.setProgressBar(trialnum/22);
    	}
	};
...
//   };
	...
	if(preview == false){
		jsPsych.init({
			...
			on_finish: function(){
				var responses = jsPsych.data.get().filter({trial_type: 'survery-text'}).csv();
                data= {
                    responses : responses,
                    rt : reactionTime,
